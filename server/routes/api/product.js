...
router.get('/item/:slug', auth, async (req, res) => {
    try {
        ...
        let regularPrice
        const updatedProducts = (productDoc) => {
            regularPrice = productDoc.price
            productDoc.regularPrice = productDoc.price
            if (req.user.subscription === 'Gold') {
                productDoc.price = productDoc.goldMemberPrice
            } else if (req.user.subscription === 'Platinum') {
                productDoc.price = productDoc.platinumMemberPrice
            } else {
                productDoc.price = null // Assuming free membership
            }
        }

        if (req.user.role !== 'ROLE ADMIN') {
            updatedProducts(productDoc)
        }
...
router.get('/list/search/:name', auth, async (req, res) => {
    try {
        ...
        const updatedProducts = (products) =>
            products.map((product) => {
                ...
                if (req.user.subscription === 'Gold') {
                    product.price = product.goldMemberPrice
                } else if (req.user.subscription === 'Platinum') {
                    product.price = product.platinumMemberPrice
                } else {
                    product.price = null // Assuming free membership
                }
                return {
                    ...product,
                    regularPrice,
...
router.get('/list', auth, async (req, res) => {
    try {
        ...
        if (userDoc) {
            const wishListQuery = getStoreProductsWishListQuery(
                userDoc.id
            ).concat(basicQuery)
            products = await Product.aggregate(
                wishListQuery.concat(paginateQuery)
            )
        } else {
...
router.get('/list/brand/:slug', auth, async (req, res) => {
    try {
        const slug = req.params.slug
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const platinumMemberPrice = req.body.platinumMemberPrice
            const goldMemberPrice = req.body.goldMemberPrice
            ...
            if (!price || !platinumMemberPrice || !goldMemberPrice) {
                return res
                    .status(400)
            ...
            if (foundProduct) {
                return res
                    .status(400)
                    .json({ error: 'This sku is already in use.' })
            }
